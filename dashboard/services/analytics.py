...
def get_all_analytics(days: int = 14) -> dict:
    ...
    try:
        ...
        buckets = aggregate_by_bucket(conn, since)
        judge = aggregate_judge(conn, since)
...
def _counterfactual_series(conn: sqlite3.Connection, since_iso: str) -> list[dict]:
    """Cumulative counterfactual delta over time (one point per cashout date)."""
    rows = conn.execute(
        "SELECT c.ts, cf.delta FROM cashouts c "
