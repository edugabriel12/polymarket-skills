...
Self-contained: the `model_log` + `predictions` schema (shared by every sport's prediction
store) is created here with raw sqlite, so the calibration core has no skill dependency.
"""
...
# The two tables calibration reads. Mirrors the shared prediction-store schema.
_DDL = """
CREATE TABLE IF NOT EXISTS predictions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
    game_slug TEXT NOT NULL, game_date TEXT, line REAL NOT NULL,
    side TEXT NOT NULL CHECK(side IN ('OVER','UNDER')),
    entry_price REAL, strategy TEXT,
    status TEXT NOT NULL DEFAULT 'PENDENTE'
        CHECK(status IN ('PENDENTE','ACERTO','ERRO','ANULADO')),
    actual_total REAL, settled_at TEXT,
    UNIQUE(game_slug, line, side)
);
CREATE TABLE IF NOT EXISTS model_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
    game_slug TEXT NOT NULL, game_date TEXT, market TEXT, line REAL,
    ref_side TEXT, ref_prob REAL, ref_price REAL, bet INTEGER NOT NULL DEFAULT 0,
    model_params TEXT, close_price REAL, actual_total REAL, actual_btts INTEGER,
    ref_outcome INTEGER, status TEXT NOT NULL DEFAULT 'PENDENTE',
...
def record_model_log(entry, db):
    con = _conn(db)
    with con:
        con.execute(
            "INSERT OR IGNORE INTO model_log(created_at,updated_at,game_slug,game_date,"
            "market,line,ref_side,ref_prob,ref_price,bet,model_params) "
            "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
            (_now(), _now(), entry["game_slug"], entry.get("game_date"), entry.get("market"),
             entry.get("line"), entry.get("ref_side"), entry.get("ref_prob"),
             entry.get("ref_price"), entry.get("bet", 0),
             json.dumps(entry["model_params"]) if entry.get("model_params") else None))
    con.close()


def record_prediction(pred, db):
    con = _conn(db)
    with con:
        cur = con.execute(
            "INSERT INTO predictions(created_at,updated_at,game_slug,game_date,line,side,"
            "entry_price,strategy) VALUES(?,?,?,?,?,?,?,?)",
            (_now(), _now(), pred["game_slug"], pred.get("game_date"), pred["line"],
             pred["side"], pred.get("entry_price"), pred.get("strategy")))
        pid = cur.lastrowid
    con.close()
    return pid


def settle_prediction(pid, actual_total, db):
    con = _conn(db)
    with con:
        con.execute("UPDATE predictions SET status='ACERTO', actual_total=?, settled_at=? "
                    "WHERE id=?", (actual_total, _now(), pid))
    con.close()
...
class TestSettleAndReport(unittest.TestCase):
    def _mlog(self, db, slug, line, prob, bet):
        record_model_log({"game_slug": slug, "game_date": "2026-06-14", "market": "TOTAL",
    ...
    def test_coverage_over_settled_games(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for i, actual in enumerate(totals):
                ...
                settle_prediction(pid, float(actual), db)
