...
Reads a predictions SQLite DB directly — works for BOTH the MLB and soccer stores
(they share the predictions(stats_log) shape). Pure stdlib, no skill imports, so it
never triggers the two skills' module collision.

Examples:
  python audit_log.py                         # all MLB predictions, full audit
  python audit_log.py --sport soccer          # all soccer predictions
...
def main() -> None:
    ...
    p.add_argument("--sport", choices=("mlb", "soccer"), default="mlb",
                   help="Which store to read (default mlb)")
