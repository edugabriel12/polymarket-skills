#!/usr/bin/env python3
"""Pure-stdlib statistical core for MLB total-runs modeling.

NO third-party imports (numpy/scipy are unavailable) — only the `math` module.
This is the only fully deterministic, offline-testable part of the skill; keep
it free of any network/IO so `test_run_distribution.py` runs with zero setup.

Model: the GAME TOTAL runs T is Negative Binomial. MLB runs are overdispersed
(variance ~ 2x mean) and zero-inflated, so plain Poisson is wrong (see
